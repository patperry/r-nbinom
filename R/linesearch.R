...
linesearch_control <- function(value_tol = 1e-4, deriv_tol = 0.9,
                               step_tol = 1e-7, step_min = 1e-10,
                               step_max = 1e10, extrap_lower = 1.1,
                               extrap_upper = 4.0, extrap_max = 0.66,
                               bisection_width = 0.66)
{
    if (!is.numeric(value_tol) || value_tol <= 0)
        stop("value of 'value_tol' must be > 0")
    if (!is.numeric(deriv_tol) || deriv_tol <= 0)
        stop("value of 'deriv_tol' must be > 0")
    if (!is.numeric(step_tol) || step_tol <= 0)
        stop("value of 'step_tol' must be > 0")
    if (!is.numeric(step_min) || step_min <= 0)
        stop("value of 'step_min' must be > 0")
    if (!is.numeric(step_max) || step_max <= step_min)
        stop("value of 'step_max' must be > step_min")
    if (!is.numeric(extrap_lower) || !(extrap_lower > 1))
        stop("value of 'extrap_lower' must be > 1")
    if (!is.numeric(extrap_upper) || !(extrap_upper > extrap_lower))
        stop("value of 'extrap_upper' must be > extrap_lower")
    if (!is.numeric(extrap_max) || !(0 < extrap_max && extrap_max < 1))
        stop("value of 'extrap_max' must be in range (0,1)")
    if (!is.numeric(bisection_width)
            || !(0 < bisection_width && bisection_width < 1))
        stop("value of 'bisection_width' must be in range (0,1)")

    list(value_tol = value_tol, deriv_tol = deriv_tol,
         step_tol = step_tol, step_min = step_min, step_max = step_max,
         extrap_lower = extrap_lower, extrap_upper = extrap_upper,
         extrap_max = extrap_max, bisection_width = bisection_width)
}
...
{
    control <- do.call("linesearch_control", control)
    ...
    if (!(control$step_min < step && step < control$step_max))
        stop("initial step must be in range (step_min, step_max)")

    gtest <- deriv * control$value_tol
    ftest <- value + step * gtest
    lower <- list(position = 0, value = value, deriv = deriv)
    upper <- lower
    interval <- c(0, step + control$extrap_upper * step)

    z <- list(step = step, value0 = value, deriv0 = deriv, ftest = ftest,
              gtest = gtest, bracket = FALSE, auxfun = TRUE, lower = lower,
              upper = upper, interval = interval, old_width1 = Inf,
              old_width2 = Inf, converged = FALSE, control=control)
    class(z) <- "linesearch"
    z
}
...
update.linesearch <- function(object, value, deriv, ...)
{
    ...
    if (value <= ftest && abs(deriv) <= -(control$deriv_tol * object$deriv0)) {
        converged <- TRUE
        ...
        stuck <- TRUE
               ...
               && diff(interval) <= control$step_tol * interval[1]) {
        warning("step size within tolerance")
        stuck <- TRUE
    } else if (step == control$step_max && value <= ftest && deriv <= gtest) {
        warning("at step_max")
        stuck <- TRUE
    } else if (step == control$step_min && (value > ftest || deriv >= gtest)) {
        warning("at step_min")
        stuck <- TRUE
    }
    ...
    update_step(object, test)
}


update_step <- function(object, test)
{
    ftest <- object$ftest
    gtest <- object$gtest
    ...
    object <- unsafe_step(object, test)
...
maybe_bisect <- function(object)
{
    ...
    if (w >= (control$bisection_width) * (object$old_width2)) {
        object$step <- l + 0.5 * (u - l)
        object$ftest <- object$value0 + object$step * object$gtest
    }

    object$old_width2 <- object$old_width1
    object$old_width1 <- w
...
unsafe_step <- function(object, test)
{
    lower <- object$lower
    upper <- object$upper
    gtest <- object$gtest
    ...
    step <- trial_value(object$interval, lower, upper, test, bracket, control)

    # update the search interval
    upd <- update_interval(lower, upper, test, step, bracket, control)

    # safeguard the step
    step <- max(control$step_min, min(control$step_max, step))
...
update_interval <- function(lower, upper, test, step, bracket, control)
{
    ...
    bracket_endpoints <- function(lower, upper) {
        list(bracket = TRUE,
    ...
    # Case U3: lower function value, derivatives same sign
        ...
        bracket_endpoints(test, lower)
    }
}


# Trial value selection (Sec. 4, pp. 298-300)
trial_value <- function(interval, lower, upper, test, bracket, control)
{
    tmin <- interval[1]
    tmax <- interval[2]
    ...
    c <- suppressWarnings(cubic_min(l, fl, gl, t, ft, gt)) # may be NaN
    q <- quadr_min(l, fl, gl, t, ft)
    s <- secant_min(l, gl, t, gt)
    ...
    # Case 3: lower function value, derivatives same sign, lower derivative
        ...
        if (bracket) {
            guard.extrap <- if (t > l)
                 function(x) min(x, t + control$extrap_max * (u - t))
            else function(x) max(x, t + control$extrap_max * (u - t))
...
quadr_min <- function(u, fu, du, v, fv)
{
    a <- v - u
    u + (du / ((fu - fv) / a + du) / 2) * a
}
...
cubic_min <- function(u, fu, du, v, fv, dv)
{
    d <- v - u
